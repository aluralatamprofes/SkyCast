...
const verificationCity = async (cityName) => {
    ...
    return createCardList(weatherData)
} 

// Função para criar um novo cartão com dados climáticos e qualidade do ar
async function createCardList(weatherData) {

    createdCities.push(weatherData);
    localStorage.setItem('createdCities', JSON.stringify(createdCities));
...
export {createdCities, createCard, displayCard };
